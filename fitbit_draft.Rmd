...
df <- read_csv("dailyActivity_merged.csv", trim_ws = TRUE, show_col_types = FALSE)
...
```{r}
df2 <- df %>% group_by(id) %>% 
  summarise(mean_total_steps = mean(total_steps), sd(total_steps), mean_calories = mean(calories), sd(calories))
```
