...
##### Percentage of users that take at least 5000 steps on average
```{r}

# Aggregate IDs by steps averages 
average_data <- daily_activity_clean %>%
  group_by(id) %>%
...
at_least_10k_avg <- sum(average_data$average_steps >= 10000) / nrow(average_data) * 100
between_5K_10K_avg <- sum(average_data$average_steps >= 5000 & average_data$average_steps < 10000) / nrow(average_data) * 100
below_5k_avg <- sum(average_data$average_steps < 5000) / nrow(average_data) * 100

# Calculate percentages for the median column
at_least_10k_med <- sum(average_data$median_steps >= 10000) / nrow(average_data) * 100
between_5K_10K_med <- sum(average_data$median_steps >= 5000 & average_data$median_steps < 10000) / nrow(average_data) * 100
below_5k_med <- sum(average_data$median_steps < 5000) / nrow(average_data) * 100
...
percentage_data$Category <- factor(percentage_data$Category, levels = c("Below 5,000", "Between 5,000 and 10,000", "At least 10,000"))

# Create a bar plot using ggplot
ggplot(percentage_data, aes(x = Category, y = Percentage_Average)) +
  geom_bar(stat = "identity", fill = "blue") +
  labs(x = "Average Total Steps", y = "Percentage Users", title = "Percentage of Users by Step Category") +
