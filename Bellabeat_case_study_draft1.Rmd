...
# Filter the Data for Irregular Sleepers
irregular_sleepers <- sleep_df[sleep_df$standard_deviation_sleep_hours > threshold, ]

# Filter the Data for Regular Sleepers
regular_sleepers <- sleep_df[!(sleep_df$standard_deviation_sleep_hours > threshold), ]
...
# Combine the irregular and regular sleepers into a single data frame for plotting
combined_data <- rbind(transform(irregular_sleepers, Group = "Irregular"),
                       transform(regular_sleepers, Group = "Regular"))

# Create the violin plot
ggplot(combined_data, aes(x = Group, y = average_sleep_hours, fill = Group)) +
  geom_boxplot(scale = "width", draw_quantiles = c(0.25, 0.5, 0.75), trim = FALSE) +
  labs(x = "Sleep Group", y = "Average Sleep Hours", title = "Comparison of Average Sleep Hours between Irregular and Regular Sleepers") +
...
```{r}
# Calculate the upper and lower bounds for outliers to remove them from regular sleepers
outlier_bounds <- boxplot.stats(combined_data$average_sleep_hours)$out

# Filter out the outliers
filtered_data <- combined_data[!(combined_data$average_sleep_hours %in% outlier_bounds), ]


# Create the violin plot
ggplot(filtered_data , aes(x = Group, y = average_sleep_hours, fill = Group)) +
...
```{r}
# Create the violin plot
ggplot(filtered_data , aes(x = Group, y = standard_deviation_sleep_hours, fill = Group)) +
  geom_violin(scale = "width", draw_quantiles = c(0.25, 0.5, 0.75), trim = FALSE) +
  geom_boxplot(width = 0.1, fill = "white", color = "black", outlier.shape = NA) +
  labs(x = "Sleep Group", y = "Standard Deviation Sleep Hours", title = "Comparison of SD Sleep Hours between Irregular and Regular Sleepers") +
  scale_fill_manual(values = c("red", "blue")) +
  theme_minimal()
```
...
```{r}
# Define the weights for average sleep duration and standard deviation (you can adjust these based on your preferences)
weight_average_sleep_hours <- 0.7
weight_standard_deviation <- 0.3

# Calculate the Sleep Consistency Index
sleepdf$Sleep_Consistency_Index <- (weight_average_sleep_hours * sleepdf$average_sleep_hours) +
                                  (weight_standard_deviation * (1 / sleepdf$standard_deviation_sleep_hours))

# Print the Sleep Consistency Index for each user
print(sleepdf$Sleep_Consistency_Index)
...
sleep_df <- daily_sleep_clean %>%
  ...
  mutate(time_difference_hours = average_time_in_bed_hours - average_sleep_hours,  # Calculate the time difference in hours
         awake_in_bed = time_difference_hours)  # Rename column "awake_in_bed"
...
```{r}
# Create a boxplot for average_sleep_hours
boxplot(sleep_df$average_sleep_hours,
        main = "Boxplot of Average Sleep Hours",
        ylab = "Average Sleep Hours")

# Calculate the median and standard deviation
median_value <- median(sleep_df$average_sleep_hours)
std_dev <- round(sd(sleep_df$average_sleep_hours), 2)

# Identify outliers
outliers <- boxplot.stats(sleep_df$average_sleep_hours)$out
...
```{r}
# Function to create boxplot and add legend
create_boxplot_and_legend <- function(column_name) {
  boxplot(sleep_df[[column_name]],
          main = paste("Boxplot of", column_name),
          ylab = column_name)
  
  median_value <- median(sleep_df[[column_name]])
  std_dev <- round(sd(sleep_df[[column_name]]), 2)
  outliers <- boxplot.stats(sleep_df[[column_name]])$out
  num_outliers <- length(outliers)
...
columns_to_analyze <- c("average_sleep_hours", "awake_in_bed")

# Loop through the columns and create boxplots with legends
for (col in columns_to_analyze) {
  create_boxplot_and_legend(col)
}
...
do next: I want to get a sense of sleep consistency for the users. Give 1o best idea to do this 
...
- Correlation Analysis: Calculate the correlation between 'average_sleep_hours' and 'standard_deviation_sleep_hours' to see if there is any relationship between the average sleep duration and its consistency.

- Sleep Consistency Index: Create a custom metric that combines both average sleep duration and standard deviation to create a Sleep Consistency Index for each user.

- Group Comparison: Compare sleep consistency metrics between different groups (e.g., previous groups, users or different age groups) to identify potential differences.

- Relationship with Average Sleep Duration: Explore how 'standard_deviation_sleep_hours' relates to 'average_sleep_hours.' Users with higher standard deviations and shorter average sleep durations might be experiencing more disrupted sleep.

- Identify Irregular Sleepers: Users with higher values of 'standard_deviation_sleep_hours' likely have more irregular sleep patterns, which may be indicative of possible sleep disturbances or lifestyle factors affecting their sleep consistency.

Percentiles: You can choose a specific percentile of the 'standard_deviation_sleep_hours' distribution as a threshold. For example, you can select the 75th percentile (as mentioned previously) to identify the top 25% of users with the highest variability.

Z-Scores: You can standardize the 'standard_deviation_sleep_hours' using Z-scores and then choose a Z-score threshold to classify irregular sleepers. A Z-score greater than a certain value (e.g., Z-score > 1 or Z-score > 2) could be considered irregular.

Expert Knowledge: If you have domain-specific knowledge or medical guidelines, you might set a threshold based on what is considered clinically significant for sleep variability.

Data-Driven Approach: Alternatively, you can use clustering or machine learning techniques to group users based on sleep patterns and identify clusters that exhibit higher variability.

The most appropriate threshold depends on the goals of your analysis and the specific questions you want to answer. You may also need to consider factors such as sample size, distribution characteristics, and the context of the data.

Once you have chosen a threshold, you can use it to create a subset of irregular sleepers and analyze their sleep patterns separately. Remember that selecting the threshold requires careful consideration and validation to ensure it aligns with the objectives of your study and provides meaningful insights into sleep consistency.
