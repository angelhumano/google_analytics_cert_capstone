...
```
If total_step is zero that means that the person did not wear the Fitbit. From our inspection above, we can see that we just need to delete the entries where total_steps is zero and will take take care of the rest.
...
# Join data sets
These data sets shared the same Id and Activity_hour, let us join them into a new data set (hourly_activity) them before we clean them.
