...
```{r}
# Example dataframe
df <- data.frame(sedentary_min_awake = c(200, 300, 400))

# Function to convert minutes to hours and minutes with two digits for minutes
convert_minutes <- function(minutes) {
  ...
  formatted_minutes <- sprintf("%02d", remaining_minutes)  # Display minutes with two digits and leading zero if necessary

  return(paste(hours, "hours", formatted_minutes, "minutes", sep = " "))
}

# Apply conversion to the sedentary_min_awake column
df$converted_time <- sapply(df$sedentary_min_awake, convert_minutes)
...
- Remember convert minutes to hours to improve readeability 
