...
t_sedentary_df <- clean_subset %>%
  ...
  summarise(average_sedentary_min_awake = mean(sedentary_min_awake),
            median_sedentary_min_awake = median(sedentary_min_awake))
...
ggplot(overall_average_df, aes(x = Percentage, y = reorder(row.names(overall_average_df), Percentage), fill = row.names(overall_average_df))) +
  ...
  xlab("Percentage") +
  ggtitle("Stacked Horizontal Bar Chart of Minutes Intensity Percentages") +
...
# Create a new dataframe with modified row names
overall_average_df <- data.frame(Minutes_Intensity = row.names(overall_average_df),
                                 Percentage = overall_average_df$Percentage)

# Create the stacked horizontal bar chart
ggplot(overall_average_df, aes(x = Percentage, y = factor(1), fill = factor(Minutes_Intensity, levels = custom_order))) +
  geom_bar(stat = "identity", width = 0.7) +
  xlab("Percentage") +
  ylab("") +
  ggtitle("Stacked Horizontal Bar Chart of Minutes Intensity Percentages") +
  scale_fill_manual(name = "Minutes Intensity", values = c("Very Active" = "red", "Fairly Active" = "orange", "Lightly Active" = "lightgreen", "Sedentary" = "lightblue"), breaks = custom_order) +
  theme_minimal() +
  theme(legend.position = "bottom", panel.grid = element_blank())
...
- stack bar for types of minutes and distance types
- Analyze sleep daily for user behavior
